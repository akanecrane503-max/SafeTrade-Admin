...
export default function WithdrawalTable({ withdrawals, loading, onView, onApprove, onReject }) {
  if (loading) {
    return (
            ...
            {withdrawals.map((withdrawal) => (
              <tr key={withdrawal.id} className="hover:bg-slate-800/30 transition-colors">
                <td className="px-4 py-3">
                  <p className="font-medium text-slate-200">{withdrawal.userName}</p>
                ...
                <td className="px-4 py-3 text-slate-400 font-mono text-xs">
                  {truncateAddress(withdrawal.destinationWallet)}
                </td>
                <td className="px-4 py-3 text-slate-400">{formatDateTime(withdrawal.createdAt)}</td>
                <td className="px-4 py-3">
