...
export default function Login() {
  ...
  const { signIn, signInWithGoogle, signingIn, error, errorType } = useAdminAuth();
  ...
  async function handleGoogleSignIn() {
    ...
    try {
      ...
      // Browser redirects to Google here — no further code runs on success.
    } catch {
      ...
      <div className="w-full max-w-sm relative">
        ...
        <div className="card p-6">
          {(error && errorType === 'pending') && (
            <div className="flex items-start gap-2.5 bg-amber-500/10 border border-amber-500/20 rounded-xl px-3 py-2.5 mb-4">
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-2.5 py-2.5 rounded-xl border border-slate-700 bg-slate-800/60 text-slate-200 text-sm font-medium hover:bg-slate-800 transition-colors disabled:opacity-50"
          >
